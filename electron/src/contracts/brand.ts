...
import { resolveBrandSourcePath } from "../main/infrastructure/runtime/runtime-paths";
...
export function getUrightBrandDataURL() {
  const sourcePath = resolveBrandSourcePath();
  const svg = sourcePath && fs.existsSync(sourcePath) ? fs.readFileSync(sourcePath, "utf8") : URIGHT_BRAND_SVG;
